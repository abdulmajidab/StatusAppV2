...
const ReplyFooterView = ({ progressIndex, messageList, onReplyButtonClick, onReplyTextChange }: ReplyFooterProps) => {
    const [keyboardPadding, setKeyboardPadding] = useState(0);
    const [showReply, setShowReply] = useState(true);

    useEffect(() => {
        let listener1 = Keyboard.addListener(Platform.OS === "ios" ? 'keyboardWillShow' : 'keyboardDidShow', onShowKeyboard);
        let listener2 = Keyboard.addListener(Platform.OS === "ios" ? 'keyboardWillHide' : 'keyboardDidHide', onHideKeyboard);
    ...
    var animationValue = new Animated.Value(-5)
    const animatedStyle = {
        ...
        <View style={[styles.parentStyle, { bottom: keyboardPadding }]}>

            {
                messageList && messageList.length > 0 && (
                    <View>
                        <Text onPress={() => Keyboard.dismiss()} numberOfLines={100000000} style={styles.messageStyle}>
                            {messageList[progressIndex]}
                        </Text>
                    </View>
                )
            }
            ...
            {
                showReply && (
                    <TouchableOpacity style={styles.replyView} onPress={() => setShowReply(false)}>
                        <Animated.View style={[styles.animatedBox, styles.imgStyle, animatedStyle]}>
