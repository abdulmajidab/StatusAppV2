...
module.exports={
  saveUserUnreadCount: async (body,isNewChat)=> {
  // Update unread count details against Room Id
  // console.log('chfkf',body.roomId);
    try {
      let listItem = (await ChatUnreadCountModel.findOne({ roomId: body.roomId,}));
      ...
      if (listItem && listItem != null) {
        if (body.chatUnreadCount.userId == listItem.chatId1) {
          ...
          // console.log(`User = ${body.chat.userName}, Room Id = ${body.roomId}, Unread Count = ${listItem}`);
  ...
  clearUserUnreadCount:async(roomId, userId)=>{
    try {
      // console.log("clearUserUnreadCount ID => ", roomId, " UserID => ", userId);
      let listItem = (await ChatUnreadCountModel.findOne({roomId: roomId,}));
      ...
      if (listItem && listItem != null) {
        if (userId == listItem.chatId1) {
          ...
          await ChatUnreadCountModel.updateOne({ roomId: roomId },{ $set: { chatId1Count: 0 } });
        } else if (userId == listItem.chatId2) {
          await ChatUnreadCountModel.updateOne({ roomId: roomId },{ $set: { chatId2Count: 0 } });
        }
      }
    } catch (error) {
